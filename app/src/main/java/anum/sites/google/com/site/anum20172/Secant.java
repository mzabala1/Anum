...
public class Secant extends AppCompatActivity {
    ...
    protected void createTable() {
        final TextView secantResultView = (TextView)findViewById(R.id.secant_result);

        final EditText x0Edit = (EditText)findViewById(R.id.x0numberSecant);
        final EditText x1Edit = (EditText)findViewById(R.id.x1numberSecant);
        final EditText tolEdit = (EditText)findViewById(R.id.tolnumberSecant);
        final EditText funcEdit = (EditText)findViewById(R.id.funcSecant);
        final EditText iterEdit = (EditText)findViewById(R.id.iternumberSecant);

        String fn =  funcEdit.getText().toString();
        ...
        try {
            ...
            TableLayout table = (TableLayout) findViewById(R.id.secant_table);
            ...
            if (iter > 0) {
                ...
                if (fx1 == 0) secantResultView.setText("Root in = " + x1);
                if (error < tol) {
                    secantResultView.setText("Approach to a root in Xn = " + x1 + "with an error of " + error);
                    return;
                }
                if ((fx1 - fx0) == 0) {
                    secantResultView.setText("Possible multiple roots.");
                    return;
                }
                secantResultView.setText("Surpassed number of iterations.");
                return;
            }
            secantResultView.setText("The number of iterations must be more than 0.");
            return;
        } catch (RuntimeException e) {
            secantResultView.setText(e.getMessage());
        }
    }
}
